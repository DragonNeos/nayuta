...
public class sceDisplay extends HLEModule {
		...
		protected void paintGL() {
	    	...
	    	if (resetDisplaySettings) {
	    		...
	    		saveGEToTexture = Settings.getInstance().readBool("emu.enablegetexture");
                        forceFalcomHD = Settings.getInstance().readBool("emu.enablefalcomhd");
	        	if (saveGEToTexture) {
	        		log.info("Saving GE to Textures");
	        	}
                        if(forceFalcomHD)
                        {
                            log.info("Falcom HD mode enabled");
                        }
	    	}
	    	...
	    	if (startModules) {
	    		saveGEToTexture = Settings.getInstance().readBool("emu.enablegetexture");
	    		if (saveGEToTexture||forceFalcomHD) {
		    	...
		    	if (saveGEToTexture && !re.isFramebufferObjectAvailable()) {
		    		...
		    		log.warn("Saving GE to Textures has been automatically disabled: FBO is not supported by this OpenGL version");
                        ...
                        if (forceFalcomHD && !re.isFramebufferObjectAvailable()) {
		    		forceFalcomHD = false;
		    		log.warn("Falcom HD mode has been automatically disabled: FBO is not supported by this OpenGL version");
		    	}
		    	isStarted = true;
	    	}
			...
			if (isUsingSoftwareRenderer()) {
	            ...
	            if (VideoEngine.getInstance().update()) {
	            	...
	            	if (saveGEToTexture && !VideoEngine.getInstance().isVideoTexture(topaddrGe)||forceFalcomHD) {
	        	...
	        	if (saveGEToTexture && !VideoEngine.getInstance().isVideoTexture(topaddrFb)||forceFalcomHD) {
    ...
    private boolean forceFalcomHD = false;
    ...
    public sceDisplay() throws LWJGLException {
        ...
        setSettingsListener("emu.enablefalcomhd", displaySettingsListener);
...
//    			log.debug("Starting Software Rendering Display Thread");
    ...
    public boolean getForceFalcomHD() {
    	return forceFalcomHD;
    }
